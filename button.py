...
class Button():
    '''
        This class is for creating buttons, there are this methods
            write_text
                Writing text in the button
            draw_button
                Make button visible for player
            pressed
                Give you information about pressing button
    '''
    ...
    def pressed(self, mouse):
        if mouse[0] > self.rect.topleft[0] and mouse[1] > self.rect.topleft[1] \
                and mouse[0] < self.rect.bottomright[0] and mouse[1] < self.rect.bottomright[1]:
            print('Clicked')
            return True
        else:
            return False
